...
/magnet command = spawns magnet object 5 units above car id 3053
...
//	#include "../include/gl_common.inc"  // pretty much only used for strtok it seems
...
#define BOT_2_NICKNAME "B3x2"
#define BOT_2_REALNAME "SA-MP Bot"
#define BOT_2_USERNAME "bot1"
...
// Connect the first bot
	...
	SetGameModeText("Stunt Race Freeroam DM Gang TDM FR");
	ShowNameTags(1);
	ShowPlayerMarkers(1);
	SetWorldTime(17);
	SetWeather(10);
...
//textdraws
	...
	Help = TextDrawCreate(4.000000, 431.500000, "~r~/MENU /C /T /DM /GOD /V ~y~/CAR /TUNE /SNOS /JUMP ~r~/RADIO~y~ /HIDEUI /KEY2 /KEYY /KEYN");
	...
	ServerIP = TextDrawCreate(486.000000, 2.000000, "~y~everystuff.net:7777"); //was 150 422
...
//				SetPlayerArmour(playerid,100.0);
			...
			if(vehicle != INVALID_VEHICLE_ID && GetVehicleModel(vehicle) == 548)
			{
				if(attached[playerid] != INVALID_VEHICLE_ID)
				{
			        new prevcar = vehicle;
				    new Float:zangle;
				    GetVehiclePos(vehicle,playerx,playery,playerz);
				    ...
				    SetVehicleZAngle(attached[playerid],zangle);
...
public OnPlayerTakeDamage(playerid, issuerid, Float: amount, weaponid)
{
	...
	if(issuerid != INVALID_PLAYER_ID)
	{
	    if(godmode[issuerid])
		{
			...
			cantgod[issuerid] = 1;
			GameTextForPlayer(issuerid,"~n~~n~~w~GOD MODE ~r~DISABLED",3000,3);
			ShowGodTD(issuerid);
		}
        if(!gTeam[playerid]) //freeroam
        {
            ...
            else // player is vulnerable
            {
                switch(weaponid)
                {
                    case 35,36,37,38,39: OPTDamt = amount/3; //reduce to 1/3
...
//			horn[playerid] = 0;
		...
		case 2: //driver
		{
            ...
            if(modelid == 548)
			{
				GameTextForPlayer(playerid,"~y~Type ~r~/magnet~n~~y~For Skycrane",3000,3);
...
//				SendClientMessage(playerid,COLOR_PINK,"{FF66FF}[TIP]:{C0C0C0} Type {FFFFFF}/radio{C0C0C0} and choose an airport to listen to REAL air-traffic-control radio audio");
				if(modelid == vHYDRA || modelid == vHUNTER)
				{
...
// 9020,9402,9437,9613,9624,9639,10402,10408,10600,10607,10801,dcmd_s
	...
	else if(dialogid == D_CAR && response)
	{
	    switch(listitem)
	    {
	        case 0: dcmd_v(playerid,params2);
	...
	else if(dialogid == D_GUNS && response)
	{
	    ...
	    switch(listitem)
	    {
			...
			case 29: id = 40; //det packs = 39
			case 30: id = 44; //night goggles
			case 31: id = 45;// thermal
			case 32: id = 4;//knife
			default: return 1;
	    }
...
dcmd_cmd(const playerid, const params[])
{
    ...
    SendClientMessage(playerid,COLOR_BRIGHTGREEN,"credits,god,register,login,ignore,pm,spawn,tele,dm,car,trailer,propertyhelp,glasses,sp,lp,key2,keyy,keyn,lock,unlock,reserve");
	SendClientMessage(playerid,COLOR_BRIGHTGREEN,"kill,toys,day,night,rain,radio,tow,flip,nos,snos,neon,tune,color,paintj,wheels,horn,putmine,putramp,putskulls,findcar,getcar,flymode");
	SendClientMessage(playerid,COLOR_BRIGHTGREEN,"beer,wine,blunt,vr,e(ject),gunship,jump,join,skin,pskin,pcolor,wrc,count,jetpack,pc,gun,fight,drunk,time,weather,piss,anims,afk");
	SendClientMessage(playerid,COLOR_BRIGHTGREEN,"magnet,stats,leave,menu,airports,stadiums,t,v,c,cities,drunk,launch,trucks,leftovers,trucker,casino,skyway,chiliad,ls,lv,sf ");
...
dcmd_magnet(playerid,params[])
{
 	...
 	else SendClientMessage(playerid,COLOR_ERROR,"{CC0000}[ERROR]:{C0C0C0} You are NOT in a CargoBob helicopter (modelid 548)");
	return 1;
	#pragma unused params
}
...
dcmd_skin(const playerid, const params[])
 {
    ...
    if(!strlen(params)) return SendClientMessage(playerid,COLOR_ERROR,"{CC0000}[ERROR]:{C0C0C0} You must provide a skinid ex: '/skin 299'. To choose by picture, type {CC0000}/pskin");
...
dcmd_xcommands(playerid,params[]) {
	...
	SendClientMessage(playerid,yellow,"adminhq,announce,(un)freeze,gethere,goto,xspec,ip,kick,ban,setwanted,wire,xjail,xunjail,nuke"); //   settime,goto,gethere,say,asay,aslap,(un)wire,awire,kick,akick,ban,aban,xspec,giveweapon,giveallweapon,xcommands,weather,eject,explode,setwanted,setallwanted");
...
public MilitaryDM(playerid)
{
	...
	if(godmode[playerid])  { godmode[playerid] = 0; ShowGodTD(playerid); }
    gTeam[playerid] = 5;
    SetPlayerArmour(playerid,100.0);
    SetPlayerInterior(playerid, 0);
    new rand = random(sizeof(gRandomMilitaryDM));
...
public FiveSeconds()
{
	foreach(Player,i)
	{
        ...
        {
            //nuke script
  			new Float:fltPlayerHealth, Float:fltTargetHP;
			GetPlayerHealth(i,fltPlayerHealth);
			fltTargetHP = fltPlayerHealth - 2;
