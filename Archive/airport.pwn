...
#define BOT_1_NICKNAME "B3x1" // Name that everyone will see
#define BOT_1_REALNAME "SA-MP Bot" // Name that will only be visible in a whois
#define BOT_1_USERNAME "bot1" // Name that will be in front of the hostname (username@hostname)
...
dcmd_admins(playerid,params[])
{
	...
	{
		if(IsPlayerXAdmin(i)) Count++;
	}
	if(!Count || !IsPlayerXAdmin(playerid)) return SendClientMessage(playerid,green,"Admins travel single-file...to hide their numbers!");
	if(Count == 1)
	{
		...
		return SendClientMessage(playerid,green,string128);
	}
	if(Count >= 1)
	{
	    ...
	    for(i = 0; i < MAX_PLAYERS; i++) if(IsPlayerConnected(i) && IsPlayerXAdmin(i))
		{
			...
			if(!First)
			{
				format(string128,sizeof(string128),"Admins Online: %s (%d),",name,Variables[i][Level]);
				First = true;
				SendClientMessage(playerid,green,string128);
			}
	        else
			{
				format(string128,sizeof(string128),"%s %s (%d)",string128,name,Variables[i][Level]);
				SendClientMessage(playerid,green,string128);
			}
	    }
	}
...
public FiveSeconds()
{
	...
	foreach(Player,i)
	{
	    if(!IsPlayerNPC(i))
        {
			...
			if(!IsPlayerXAdmin(i) && (health > 101 || armour > 101))
			{
