...
- added /caralarm
- added /lights
- added rudimentary bad-vehicle 'bounceback' to safe zones
-------------------------------------------------------------------
...
#define BOT_1_NICKNAME "samp1" // Name that everyone will see
...
new bool:helicopter[MAX_PLAYERS];
...
//	ResetPlayerWeapons(playerid);
	...
	if(killerid == INVALID_PLAYER_ID && !sentdeathmsg[playerid])
	{
	...
	else // PvP
	{
	    ...
	    switch(reason)
	    {
	        ...
	        case 50,51:
	        {
				...
				{
//# helikill
					switch(pmodelid[killerid])
					{
					    case 548,425,417,487,488,497,563,447,469:
					    {
					        helicopter[killerid] = true;
					    }
					
					}
					if(helicopter[killerid] == false)
					{
	                    format(string128,sizeof(string128),"[FAKE HELIKILL] Blocked [%i]%s from fake helikilling.",playerid,pname[playerid]);
					}
				    helikills[killerid]++;
				    if(helikills[killerid] > 2)
				    {
						format(string128,sizeof(string128),"[helikill] Kicked [%i]%s for too much helikilling. Victim: [%i]%s",killerid,pname[killerid],playerid,pname[playerid]);
//						SendClientMessageToAll(COLOR_YELLOW,string128);
						printf(string128);
      					#if defined IRC_ECHO
                		IRC_GroupSay(groupID, IRC_CHANNEL, string128);
...
public OnPlayerTakeDamage(playerid, issuerid, Float: amount, weaponid)
{
	...
	{
	    if(godmode[issuerid] && !agodmode[issuerid])
		{
...
//					    GameTextForPlayer(playerid,string32,3000,3);
		...
		{
			if(OPTDhealth-OPTDamt < 0.1 && !gTeam[playerid] && !gTeam[issuerid]) // FR player has killed FR player
			{
			    if(!sentdeathmsg[playerid])
			    {
		            if(GetPlayerWantedLevel(playerid) > 5)
					{
						GivePlayerMoney(issuerid,2000); //killing wanted = extra money
						SetPlayerWantedLevel(playerid,0); //set dead 6-star person to 0 star
					}
					else //give wanted to killer
					{
						new wanted;
						wanted = GetPlayerWantedLevel(issuerid);
					    if(wanted < 6) SetPlayerWantedLevel(issuerid,wanted+1); //make sure you dont go over 6-stars
					}
...
dcmd_say(playerid,params[])
{
	...
	else format(string128,sizeof(string128),"%s -- %s --",pname[playerid],params);
...
{
	//reserved
	SendClientMessage(playerid, COLOR_ERROR, "Reserved for future use...");
	return 1;
	#pragma unused playerid
	#pragma unused params
}
...
public FiveSeconds()
{
	...
	foreach(Player,i)
	{
	    if(!IsPlayerNPC(i))
        {
			...
			if((pstate[i] != PLAYER_STATE_WASTED && !IsPlayerXAdmin(i)) && (health > 101 || armour > 101))
			{
			    if(!IsPlayerInAnyVehicle(i))
			    {
					format(string128,sizeof(string128),"[hack][WARN] - [%i]%s flagged for health/armor hacks! Hlth:%.0f Armr:%.0f",i,pname[i],health,armour);
					SendClientMessage(i,COLOR_ERROR,string128);
					cheatflag[i]++;
				}
				if(cheatflag[i] == 5)
				{
				    format(string128,sizeof(string128),"[kick] [%i]%s kicked for too many cheat violations",i,pname[i]);
...
IRCCMD:iwhoson(botid, channel[], user[], host[], params[])
{
	if (IRC_IsHalfop(botid, channel, user))
	{
		...
		foreach(Player,i)
		{
		    ...
		    format(string128, sizeof(string128), "[%i]%s - %d - %s",i,pname[i],money,pip);
